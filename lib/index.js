...
class ElectronDownloader {
  ...
  get cache () {
    if (process.env.NPM_CONFIG_ELECTRON_CACHE || process.env.ELECTRON_CACHE) {
      return process.env.NPM_CONFIG_ELECTRON_CACHE || process.env.ELECTRON_CACHE
    }
  ...
  checkForCachedZip (cb) {
    var onlyCache = !!process.env.NPM_CONFIG_ELECTRON_ONLY_CACHE || !!process.env.ELECTRON_ONLY_CACHE || this.opts.onlyCache
    pathExists(this.cachedZip).then(exists => {
      if (exists && !this.force) {
        debug('zip exists', this.cachedZip)
        this.checkIfZipNeedsVerifying(cb)
      } else if (onlyCache) {
        return cb(new Error('Failed to find Electron in cache ' + this.opts.cache + ' (cache forced)'))
      } else {
