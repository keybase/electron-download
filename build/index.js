...
var ElectronDownloader = function () {
  ...
  _createClass(ElectronDownloader, [{
    ...
    value: function checkForCachedChecksum(cb) {
      ...
      pathExists(this.cachedChecksum).then(function (exists) {
        if (exists) {
          ...
          if (_this.opts.requireChecksum) {
            return cb(new Error('required checksum file does not exist: ' + _this.cachedChecksum));
          }
          if (_this.tmpdir) {
    ...
    key: 'cachedChecksum',
    get: function get() {
      return this.opts.requireChecksum || path.join(this.cache, this.checksumFilename + '-' + this.version);
    }
  }, {
    key: 'cachedZip',
    get: function get() {
